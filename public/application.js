...
var onUpdateGame = function onUpdateGame(cellId, letter, boolean) {
  api.updateGame(cellId, letter, boolean).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
};
...
var checkForDraw = function checkForDraw() {
  if (store.game.cells[0] !== '' && store.game.cells[1] !== '' && store.game.cells[2] !== '' && store.game.cells[3] !== '' && store.game.cells[4] !== '' && store.game.cells[5] !== '' && store.game.cells[6] !== '' && store.game.cells[7] !== '' && store.game.cells[8] !== '' && store.game.over !== true) {
...
var onDisplayGame = function onDisplayGame() {
  api.displayGame().then(ui.displayGameSuccess).catch(ui.displayGameFailure);
};
...
var gamesEvents = __webpack_require__(3);
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  authApi.signIn(data).then(authUi.signInSuccess).then(gamesEvents.onNewGame).catch(authUi.signInError);
};
...
var newMove = function newMove(move) {
  return $.ajax({
...
var updateGame = function updateGame(cellId, letter, boolean) {
  var data = {
    "game": {
      "cell": {
        "index": cellId,
...
var displayGameSuccess = function displayGameSuccess(data) {};
var displayGameFailure = function displayGameFailure(response) {};
...
module.exports = {
  ...
  displayGameSuccess: displayGameSuccess,
  displayGameFailure: displayGameFailure,
